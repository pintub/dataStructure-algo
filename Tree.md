...
- Delete node to BST (`Tricky`)
  ...
  - Delete node w/ two child
    ...
    - [Approach-2](https://www.geeksforgeeks.org/binary-search-tree-set-2-delete/)(Optimized to avoid above last recursion) - Find parent-of-Successor & successor. Copy successor node's value into deleted node.Then make successor node's child as parent-of-Successor's child
...
- SerializeAndDeserializeBinaryTree297 (Serialization)
  ...
  - `Solution`: Level-order solution is intuitive. But you will have to fill all tree as complete Tree, by filling vacant places as X, which takes more space. So `Use preoder + with X for null` in both serialize() and deserialize() methods. Similarity - Other question need PreOrder + InOrder as list were not storing null, if you store null preorder is sufficient
 <pre>
 * 1---2---4---5---null
 * |   |   |   |
 * nul nul nul nul
