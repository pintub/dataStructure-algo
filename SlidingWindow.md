...
- Check code a template implementation of sliding window
  - [Fixed](Practice/src/main/java/com/p2/slidiing/window/fixed/MaxSumSubArrayOfSizeK.java)
  - Variable problems has mostly 2 if-else branches, i.e. One for valid window, other for invalid window. If invalid window reached, move left pointer to make window valid.
...
- Choices of popular Memorization DS
  ...
  - `Queue` : As we traverse through array and an element comes into queue first also gets removed first.<br/> 
     Dequeue is also used if both side operation required(Refer `SlidingWindowMaximum239`)
- Exception Questions
  - `SubArraySumEqualsKWithNegativeNumbers560`, Not sliding window
